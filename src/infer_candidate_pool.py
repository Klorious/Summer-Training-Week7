...
from infer import MODEL_SPECS, load_checkpoint
...
def validate_predictions(results_frame):
...
def build_distribution_table(results_frame):
    records = []
...
def main():
    ...
    validate_predictions(results_frame)

    output_path = Path(args.output_csv)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    results_frame.to_csv(output_path, index=False)

    distribution_frame = build_distribution_table(results_frame)
